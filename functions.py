...
def plot_mesh_and_points(mesh, points,figx=14,figy=8, saveMyFigure=False):
    ...
    ax.plot_trisurf(x_coords, y_coords, faces, z_coords, linewidth=0.2, edgecolor='None', alpha=1.0)
    ...
    ax.set_zlim([0,3.0*np.max(z_coords)])
    ...
    ax.yaxis.set_major_locator(MaxNLocator(nbins=4,integer=True, prune='both'))
    ax.zaxis.set_major_locator(MaxNLocator(nbins=2,integer=True, prune='both'))
    # Save figure if user prompts
    if(saveMyFigure):
        plt.savefig('Figures/samplingPoints.eps',dpi=600)
    plt.show()
#
...
# Slice an OBJ
#
def slice_obj(obj_file, z_level):
    vertices, faces = load_obj(obj_file)

    sliced_faces = []
    for face in faces:
        z_values = vertices[face, 2]
        if np.all(z_values <= z_level) or np.all(z_values >= z_level):
            continue
        elif np.any(np.logical_and(z_values >= z_level, z_values <= z_level)):
            # Interpolate the intersection point
            weights = (z_level - z_values[0]) / (z_values[2] - z_values[0])
            intersection_point = vertices[face[0]] + weights * (vertices[face[2]] - vertices[face[0]])

            # Add the intersection point to the vertices
            new_vertex_index = len(vertices)
            vertices = np.vstack([vertices, intersection_point])

            # Replace the third vertex of the original face with the new intersection point
            face[2] = new_vertex_index

        sliced_faces.append(face)

    return vertices, sliced_faces
